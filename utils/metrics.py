    ...
    def plot(self, save_dir='', names=()):
        try:
            ...
            sn.heatmap(array, annot=self.nc < 30, annot_kws={"size": 8}, cmap='Blues', fmt='.2f', square=True, cbar=False,
                       xticklabels=names + ['BG FP'] if labels else "auto",
                       yticklabels=names + ['BG FN'] if labels else "auto").set_facecolor((1, 1, 1))
            fig.axes[0].set_xlabel('True')
            fig.axes[0].set_ylabel('Predicted')
            fig.savefig(Path(save_dir) / 'confusion_matrix.png', dpi=600)
